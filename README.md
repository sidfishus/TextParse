...
OR
2.1. Compare against string 'hello'
AND
2.2. Compare against a single character in the sequence 1,2, or 3.

The second parse statement is a combination of statements but because the parse statement list class ('StatementList') derives from the same interface as the individual parse statements, the parse statement list type can be used anywhere a parse statement is required without the caller needing to have any knowledge of how it is implemented
